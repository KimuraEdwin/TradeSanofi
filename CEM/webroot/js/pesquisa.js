$(document).ready(function () {
    ...
    var pesquisar = function () {
		...
		$("#btn-pesquisa-fechar").css({display:"block"});
		$("#txt-pesquisa").css({left:"200px", borderRadius:"5px 0px 0px 5px"});
		$("#btn-pesquisa").css({display:"none"});

        var HTML = "";
        var p = $input.val();
        var area = $("#resultados-pesquisa");
		var areaHeroi = $("#area-heroi");
        var lista = [
			 {
			   "arquivo": "aas.html",
			   "titulo": "aas",
        ...
        $.ajax({
			...
			resp.result.forEach(function (v) {
				if(resp.result.length == 1 && v.heroi != ""){
					areaHeroi.html("<img src='../../webroot/img/heroi/heroi-pesquisa/" + v.heroi + "' />")
				}	
	...
	$("#btn-pesquisa-fechar").click(function(){
		$("#btn-pesquisa").css({display:"block"});
		$("#btn-pesquisa-fechar").css({display:"none"});
		$("#txt-pesquisa").css({left:"232px", borderRadius:"0px 5px 5px 0px"});
		$input.val("");
	})
